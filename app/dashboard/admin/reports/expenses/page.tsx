...
export default function ExpensesReportPage() {
  ...
  const downloadReport = async () => {
    ...
    const url = `/api/reports/expenses?from=${fromStr}&to=${toStr}`;
    ...
    try {
      ...
      a.href = downloadUrl;
      a.download = `expenses-report-${fromStr}-to-${toStr}.xlsx`;
      ...
      <header className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur-md px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900 leading-none">Expenses Report</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="bg-white border border-blue-100 rounded-2xl p-6 shadow-sm shadow-blue-100/50 mb-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">System Expenses</h2>
              <p className="text-sm font-medium text-slate-500 mt-1">
                Download a detailed breakdown of all fuel, food, and maintenance costs.
              </p>
            </div>
          </div>
        </motion.div>
